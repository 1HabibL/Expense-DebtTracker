...
if(savedFilters){
    ...
    const fullyfilteredExpenses = parsedExpenses.filter((x) => {
        return (!filterName || x.expenseName.toLowerCase() === filterName) &&
            (!filterCategory || x.category.toLowerCase() === filterCategory) &&
    ...
    className={`my-4 shadow-lg rounded-2xl p-4 bg-[#f8f9fa] transform transition-all duration-500 ease-in-out origin-top overflow-hidden
     ${isHidden 
      ...
      : 'opacity-100 translate-y-0 scale-y-100  pointer-events-auto h-auto'
            ...
            <div id="SectionTwo" className='flex'>
                ...
                <div id="ExpenseDateDesigns" className='flex flex-col'>
                 ...
                 <div className="flex" id="SectionThree">
                    <div id="AdditionalNotesDesign" class="flex flex-col">      
        ...
        <input id="filterName" type="text" value={filterName} onChange={(e) => setFilterName(e.target.value.toLowerCase())} className='rounded-l shadow-md bg-white focus:outline-none fcous-ring-2'></input>
            </div>
            ...
            <ul className='flex flex-col justify-center items-center w-6/7'>
                ...
                Array.isArray(expenses) && (filteredExpenses.length > 0 ? filteredExpenses : expenses).map((expense, index) => (
                    <li className='text-2xl my-2 flex bg-[#f8f9fa] px-4  py-2 items-center gap-4 shadow-lg rounded-xl hover:shadow-2xl transition-shadow duration-300 w-[100%]' key={index}>
                        ...
                        <button className="bg-red-300 text-white p-2 rounded-lg shadow-med hover:bg-red-600 transition flex items-center justify-center"onClick={() => handleEdit(index)}><span className="material-symbols-outlined">delete</span></button>
                        <button className="bg-blue-300 text-white p-2 rounded-lg shadow-med hover:bg-red-600 transition flex items-center justify-center"onClick={() => handleDelete(index)}><span className="material-symbols-outlined">edit</span></button>
                    </li>
                ))
                }
            </ul>
